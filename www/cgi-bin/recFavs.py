...
query = "SELECT Data, Title, Posttime, UnameSent FROM Messages WHERE mID IN (SELECT MessageID FROM Favorites WHERE Uname='test4');" #UNSANITIZED UNTIL FURTHER NOTICE
...
# # result = result[0]
for i in range(len(result)):
	jsonreturn[str(i)] = {"Data": result[i][0], "Title": result[i][1], "Posttime": result[i][2], "UnameSent": result[i][3]}
conn.close()
